...
* **Identify Decisions:** What architectural, technical, or design decisions were made? What alternatives were considered? What was the rationale? (e.g., "Chose PostgreSQL over MongoDB for ACID compliance", "Selected JWT over sessions for stateless auth")
...
* *Example Entry in `docs/raw_reflection_log.md`:*
    ...
    Decisions:
    - Chose JWT over sessions for stateless authentication to support microservices architecture
    - Selected `jose` library over `jsonwebtoken` for better JWK support and TypeScript definitions
    - Decided to implement 5-minute JWKS cache with explicit bust mechanism for performance vs. security balance
