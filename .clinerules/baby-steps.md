...
## Relationship with Task Planning

Baby Steps™ operates at the execution level within each subtask. When working on large projects that require task planning (see new-task-automation.md), use this workflow:

1. **Planning Phase**: Break down the overall task into logical subtasks
2. **Execution Phase**: Apply Baby Steps™ within each subtask
3. **Hierarchy**: Task Planning → Subtasks → Baby Steps™ within each subtask

**Example**:
- Task Planning identifies: "Implement user authentication" (subtask)
- Baby Steps™ executes it: Create schema → Add validation → Hash password → Create endpoint → Add tests (each as a single step)

This approach combines strategic planning (macro) with methodical execution (micro).
